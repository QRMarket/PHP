...
final class Validate{
	...
	private function checkIsCharacterLength($value,$min = null,$max = null){
		$val = mb_strlen(trim($value),Config::CHARACTER_SET);
		if($min != null) {
			if($val < $min) return 1;
		}
		if($max != null) {
     ...
     * @return boolean
     */
    public function checkIsDate($value,$separator = '/',$enAzTarih=null,$enCokTarih=null){
        if(preg_match('%^([12][0-9]|0[1-9]|3[01])[' . $separator . '](0[1-9]|1[012])[' . $separator . '](19|20)\d\d$%', $value)){
    
            $format = 'd'.$separator.'m'.$separator.'Y';
            $GIRILENZAMAN = \DateTime::createFromFormat($format, $value);
            	
            if ($GIRILENZAMAN!==FALSE && $GIRILENZAMAN->format($format)===$value){               
                if($enAzTarih instanceof \DateTime && $enCokTarih instanceof \DateTime){                     
                    if( !($GIRILENZAMAN->getTimestamp()>$enAzTarih->getTimestamp() && $GIRILENZAMAN->getTimestamp()<$enCokTarih->getTimestamp())){
                        if($GIRILENZAMAN->getTimestamp()>$enCokTarih->getTimestamp())
                           return 1;
                        else 
                            return 0;                        
                    }                    
                }else{
                    if(!is_null($enAzTarih)){
                        if($GIRILENZAMAN->getTimestamp()<$enAzTarih->getTimestamp()){
                            return 0;
                        }
                    }
                    if(!is_null($enCokTarih)){
                        if($GIRILENZAMAN->getTimestamp()>$enCokTarih->getTimestamp()){
                            return 1;
                        }
                    }
